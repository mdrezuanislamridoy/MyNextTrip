...
export default function AdminProfile() {
  ...
  const handleCoverPhotoChange = async (e) => {
    ...
    if (photo) {
      ...
      const url = res?.data?.user?.coverPic || URL.createObjectURL(photo);
      setCoverPhoto(url);
    }
  };
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-4 sm:p-6">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div
          className="w-full h-48 sm:h-64 md:h-80 relative bg-cover bg-center bg-no-repeat shadow-lg rounded-lg sm:rounded-xl overflow-hidden"
          style={{
            backgroundImage: `url(${coverPhoto})`,
          }}
        >
          <label className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-white/90 hover:bg-white transition rounded sm:rounded-lg p-2 sm:p-3 shadow-md cursor-pointer z-20 border border-gray-200 flex items-center gap-1 sm:gap-2">
            <FontAwesomeIcon
              icon={faCamera}
        ...
        {/* Profile Header Section - Responsive layout */}
